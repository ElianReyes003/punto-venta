        ...
        <div class="p-4">
		   ...
		   <div class="bg-white rounded-lg shadow-lg p-4">
			<div class="flex justify-center">
				<h1 class="text-center font-bold md:text-[2rem]">Agregar Unidad</h1>
			</div>
			<form action="{{ route('unidad.insertar') }}" method="post" class="grid md:grid-cols-3 grid-cols-1 gap-6 p-5">
            @csrf   
         
            <div>
                    <label for="" class="block mb-2 text-sm font-medium text-gray-900">Unidad</label>
                    <input  name="nombreUnidad"  type="text" id="" class="bg-blue-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-400 focus:border-blue-400 block w-full p-2.5" placeholder="" required>
                </div>
		   ...
		   <div class="bg-white rounded-lg shadow-lg mt-10">
            ...
            <div class="flex justify-center">
				<h1 class="text-center font-bold md:text-[2rem] mt-5">Lista de Unidades</h1>
			</div>
			...
			<table id="tabla-unidad" class="w-full table-auto mt-[1rem]"  class="display nowrap" width="90%">
				<thead class="text-center">
					<tr class="h-24 text-center">
                    <th class="linea-izquierda border-gray-300">Nombre de Unidad</th>
                        <th>Opciones</th>
					</tr>
					<tbody>
						...
						<tr class="h-20">
							...
							<td class="items-center flex justify-center">
                                <div class="flex justify-between">
                                    <a href="{{route('unidad.mostrarPorId', $dato->pkUnidad)}}" class="flex mr-5 w-11 h-11 items-center mt-2 bg-blue-500 p-2 text-base font-medium text-white rounded-lg hover:bg-blue-400">
                                    ...
                                    <form action="{{ route('unidad.baja', $dato->pkUnidad) }}" method="POST" class="inline">
                                        @csrf
   ...
   <script  src="https://cdn.datatables.net/plug-ins/1.13.7/api/fnMultiFilter.js"></script>
    ...
    <script>
        ...
        $(document).ready(function () {
            tableUnidad=$('#tabla-unidad').DataTable({
                "language": {
                "search": "Buscar:",
            ...
            e.preventDefault();
            tableUnidad.page('previous').draw(false);
            });

            // Agrega evento de clic al botón Next
            $('#nextBtn').on('click', function(e) {
            e.preventDefault();
            tableUnidad.page('next').draw(false);
            });
               
            $('#busqueda').on('keyup', function (e) {
            var filtroBusqueda = $('#busqueda').val();
            tableUnidad.search(filtroBusqueda).draw();
        });
    });
